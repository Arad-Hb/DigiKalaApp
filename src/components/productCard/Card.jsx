  ...
  return (
    ...
    <div className={`${styles.cardContainer}`}>
        <div className={`${styles.header}`}>
          ...
          <NavLink url={`/products/singleProduct/${product.id}`} key={GetID()}><Image src={product.indexImageUrl} alt={product.name} width={150} height={190}  className={`${styles.image}`}/></NavLink>
        </div>
        ...
        <div className={`${styles.footer}`}>
          {product.stock===0?<label className={`${styles.outOfStockLabel}`}>ناموجود</label>:<AddButton product={product} key={GetID()}/>}
        </div>
    </div>
